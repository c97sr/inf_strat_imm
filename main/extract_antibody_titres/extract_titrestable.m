function [TSubset] = extract_titrestable(data_time, data_strain, data_title, data_age )
...
TSubset = [TSubset T1];
TSubset = [TSubset tmpTitresSubset(:,data_time)];
T1 = table(str2num(char(table2cell(tmpTitresSubset(:,data_age)))));
T1.Properties.VariableNames{'Var1'} = data_age;
TSubset = [TSubset T1];
T1 = table(str2num(char(table2cell(tmpTitresSubset(:,data_strain)))));
T1.Properties.VariableNames{'Var1'} = data_strain;
TSubset = [TSubset T1];

titres = [];
for i=1:length(table2array(TSubset(:,data_strain)))
   titres(i) = round(log2(table2array(TSubset(i,data_strain)))-log2(5));
end
T1 = table(titres');
